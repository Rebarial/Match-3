...
public static class GameObjectFactory
{
    ...
    public static Dictionary<string, int> OrientationTypeDict = new Dictionary<string, int>()
    {
        { "horizontal", 0 },
        { "vertical", 1 },
    };
    ...
    public static void DeskController(int tickCounter)
    {
        ...
        if (State == GameState.AfterSwap)
        {
            StartDestroy(tickCounter);
        }
    ...
    private static void LineDetionationAnimation(int tickCounter)
    {
        ...
        {
            // Анимация распространения волны от центра линии
            double progress = 1.0 - (double)(tickAnimationEnd - tickCounter) /
                (tickAnimationEnd - (tickAnimationEnd - MyMathHelper.OneSecTicks(GameConfig.GameLoopIntervalMs) / 2));
            ...
            foreach (var lineActive in linesActive)
            {
                ...
                {
                    // Анимация горизонтальной волны
                    int centerX = coords.x;
                    int waveRadius = (int)(progress * GameConfig.BoardSize);

                    // Подсвечиваем элементы, которые будут уничтожены
                    for (int i = 0; i < GameConfig.BoardSize; i++)
                    {
                        double distance = Math.Abs(i - centerX);
                        if (distance <= waveRadius)
                        {
                            GameObjectClass element = gameObjectBoard[i, coords.y];
                            if (element != null && element != lineActive)
                            {
                                // Анимация пульсации для элементов в радиусе волны
                                double pulseScale = 1 + 0.2 * Math.Sin(progress * Math.PI * 4);
                                ...
                                // Изменяем прозрачность в зависимости от расстояния
                                double opacity = 1 - (distance / GameConfig.BoardSize) * 0.5;
                    ...
                    double lineScale = 1 + progress * 0.5;
                    lineActive.GetElement().RenderTransform = new ScaleTransform(lineScale, 1);
                    lineActive.GetElement().RenderTransformOrigin = new Point(0.5, 0.5);
                    lineActive.GetElement().Opacity = 1 - progress * 0.3;
                }
                else // vertical
                {
                    // Анимация вертикальной волны
                    ...
                    for (int i = 0; i < GameConfig.BoardSize; i++)
                    {
                        double distance = Math.Abs(i - centerY);
                        if (distance <= waveRadius)
                        {
                            GameObjectClass element = gameObjectBoard[coords.x, i];
                            if (element != null && element != lineActive)
                            {
                                // Анимация пульсации для элементов в радиусе волны
                                double pulseScale = 1 + 0.2 * Math.Sin(progress * Math.PI * 4);
                    ...
                    // Анимация самой линии
                    double lineScale = 1 + progress * 0.5;
                    lineActive.GetElement().RenderTransform = new ScaleTransform(1, lineScale);
                    lineActive.GetElement().RenderTransformOrigin = new Point(0.5, 0.5);
                    lineActive.GetElement().Opacity = 1 - progress * 0.3;
                }
            }
    ...
    {
        scale = 1;
        int animationTime = MyMathHelper.OneSecTicks(GameConfig.GameLoopIntervalMs)/2;
        ...
        if (tickCounter >= tickAnimationEnd)
        {
            foreach (var bomb in bombs)
            {
                ...
                for (int i = startX; i <= endX; i++)
                {
                    ...
                    {
                        GameObjectClass element = gameObjectBoard[i, j];
            ...
            else State = GameState.AfterSwap;
        }
        else
        {
            foreach (var bomb in bombs)
    ...
    {
        scale = 1;
        int animationTime = MyMathHelper.OneSecTicks(GameConfig.GameLoopIntervalMs) / 4;
        tickAnimationEnd = MyMathHelper.TickAddition(tickCounter ,animationTime);
    ...
    private static List<GameObjectClass> bombs = new List<GameObjectClass>();

    private static List<GameObjectClass> lines = new List<GameObjectClass>();
    private static List<GameObjectClass> linesActive = new List<GameObjectClass>();
    ...
    private static void SwapObjects(int tickCounter)
    {
        ...
        else if (tickCounter >= tickAnimationEnd)
        {
            ...
            int selectedObjectX  = selectedObject.GetCoords().x;
            int selectedObjectY  = selectedObject.GetCoords().y;
            
            int targetObjectX  = targetObject.GetCoords().x;
            int targetObjectY  = targetObject.GetCoords().y;
            ...
            if (dieGameObjects.Any())
            {
                ...
                {
                    new BonusData { ElementToBonus = targetObject, IsHorizontalBonus = false, IsVerticalBonus = false, IsHorizontalLineDestroy = false, IsVerticalLineDestroy = false },
        ...
        else
        {
            ...
            int xdif = selectedObject.GetCoords().x - targetObject.GetCoords().x;
            int ydif = selectedObject.GetCoords().y - targetObject.GetCoords().y;
    ...
    public static void CheckAndSwapWithSelected(GameObjectClass firstElement, GameObjectClass SecondElement)
    {
        ...
        if (Math.Abs(coordsFirstElement.x - coordsSecondElement.x) == 1 &&
            Math.Abs(coordsFirstElement.y - coordsSecondElement.y) == 0) relative =  true;
        
        if (Math.Abs(coordsFirstElement.x - coordsSecondElement.x) == 0 &&
            Math.Abs(coordsFirstElement.y - coordsSecondElement.y) == 1) relative =  true;
    ...
    private static void DropObjects(int tickCounter)
    {
        if (dropMatrix.Count > 0)
        {
            if (tickAnimationEnd < 0)
            {
                ...
                {
                    for (int i = gameObjectCoords.Item2; i > 0; i--)
                    {
                    ...
                    int newGameObjectClass = GameHelper.getRandomNumber();
                    
                    intBoard[gameObjectCoords.Item1, 0] = newGameObjectClass;
                    
                    gameObjectBoard[gameObjectCoords.Item1, 0] =
                        CreateGameObjectByIndexType(newGameObjectClass, gameCanvas, gameObjectCoords.Item1,
                            -1);
            ...
            else if (tickCounter >= tickAnimationEnd)
            {
                foreach (var gameObjectCoords in dropMatrix[0])
                {
                    for (int i = 0; i <= gameObjectCoords.Item2; i++)
                    {
    ...
    private static void DestroyObject(int tickCounter)
    {
        ...
        if (tickCounter >= tickAnimationEnd)
        {
            ...
            {
                Game.Score += 1;
                DeleteElement(gameObject); //gameCanvas.Children.Remove(gameObject.GetElement());
            }
    ...
    private static void CreateBonuses()
    {
        if (bonusGameObjects.Any())
        {
            foreach (var bonus in bonusGameObjects)
            {
                intBoard[bonus.x, bonus.y] = bonus.element_type;
                gameObjectBoard[bonus.x, bonus.y] = CreateGameObjectByIndexType(bonus.element_type, gameCanvas, bonus.x, bonus.y);
            }
        }
    }
    ...
    private static void CheckDestroy()
    {
        ...
        for (int i = 0; i < GameConfig.BoardSize; i++) 
        {
            ...
            for (int j = 0; j < GameConfig.BoardSize; j++) 
            {
                if (intBoard[j, i] == lastElementType)  
                    currentTypeCount++;
                else
                {
                    if (currentTypeCount >= 3)
                    {
                        bonusGameObjects.Add((j-1, i, currentTypeCount, OrientationTypeDict["horizontal"], lastElementType));
                    }
                    currentTypeCount = 1;
                }
                ...
                if (currentTypeCount >= 3)
                {
                    for (int k = j; k > j - currentTypeCount; k--)
                    {
                        var gameObjectToDie = gameObjectBoard[k, i];
                        gameObjectToDie.PlayDieAnimation(OrientationTypeDict["horizontal"]); 
                        if (!dieGameObjects.Contains(gameObjectToDie))
                        {
                            dieGameObjects.Add(gameObjectToDie);
                            if (lines.Contains(gameObjectToDie)) linesActive.Add(gameObjectToDie);
                        }
                    }
                }

                lastElementType = intBoard[j, i];
            }
            if (currentTypeCount >= 3)
            {
                bonusGameObjects.Add((GameConfig.BoardSize-1, i, currentTypeCount, OrientationTypeDict["horizontal"], lastElementType));
            }
        }
        ...
        for (int i = 0; i < GameConfig.BoardSize; i++)
        {
            ...
            for (int j = 0; j < GameConfig.BoardSize; j++)
            {
                if (intBoard[i, j] == lastElementType) currentTypeCount++;
                else
                {
                    if (currentTypeCount >= 3)
                    {
                        bonusGameObjects.Add((i, j-1, currentTypeCount, OrientationTypeDict["vertical"], lastElementType));
                    }
                    currentTypeCount = 1;
                }
                ...
                if (currentTypeCount >= 3)
                {
                    for (int k = j; k > j - currentTypeCount; k--)
                    {
                        var gameObjectToDie = gameObjectBoard[i, k];
                        gameObjectToDie.PlayDieAnimation(OrientationTypeDict["vertical"]);
                        if (!dieGameObjects.Contains(gameObjectToDie))
                        {
                            dieGameObjects.Add(gameObjectToDie);
                            if (lines.Contains(gameObjectToDie)) linesActive.Add(gameObjectToDie);
                ...
                lastElementType = intBoard[i, j];
            }
            if (currentTypeCount >= 3)
            {
                bonusGameObjects.Add((i, GameConfig.BoardSize-1, currentTypeCount, OrientationTypeDict["vertical"], lastElementType));
            }
        }
    }
    ...
    {
        GameObjectClass[,] result = new GameObjectClass[GameConfig.BoardSize, GameConfig.BoardSize];
        
        for (int i = 0; i < GameConfig.BoardSize; i++)
        {
            for (int j = 0; j < GameConfig.BoardSize; j++)
            {
                int indexValue = indexBoard[i, j];
    ...
    private static Dictionary<int, string> spriteTypeDict = new Dictionary<int, string>()
    {
        { 1, "blue" },
        { 2, "green" },
        { 3, "orange" },
